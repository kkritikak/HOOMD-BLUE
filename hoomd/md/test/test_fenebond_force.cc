...
typedef std::function<std::shared_ptr<PotentialBondFENE>(std::shared_ptr<SystemDefinition> sysdef)>
    bondforce_creator;
...
void bond_force_basic_tests(bondforce_creator bf_creator,
    ...
    fc_6->compute(0);
        ...
        MY_CHECK_CLOSE(Scalar(1. / 3.)
                           * (h_virial_4.data[0 * pitch + 2] + h_virial_4.data[3 * pitch + 2]
        ...
        MY_CHECK_CLOSE(Scalar(1. / 3.)
                           * (h_virial_4.data[0 * pitch + 3] + h_virial_4.data[3 * pitch + 3]
        ...
        MY_CHECK_CLOSE(Scalar(1. / 3.)
                           * (h_virial_4.data[0 * pitch + 4] + h_virial_4.data[3 * pitch + 4]
        ...
        MY_CHECK_CLOSE(Scalar(1. / 3.)
                           * (h_virial_4.data[0 * pitch + 5] + h_virial_4.data[3 * pitch + 5]
    ...
    fc_4->setParams(
        0,
        fene_params(Scalar(1.5), Scalar(1.75), Scalar(pow(1.2, 12.0)), Scalar(pow(1.2, 6.0))));
    ...
    for (unsigned int i = 0; i < M; i++)
        for (unsigned int j = 0; j < M; j++)
            for (unsigned int k = 0; k < M - 1; k++)
                {
                sysdef->getBondData()->addBondedGroup(
                    Bond(0, i * M * M + j * M + k, i * M * M + j * M + k + 1));
                }
    ...
    fc2->compute(0);
        ...
        for (unsigned int i = 0; i < N; i++)
            ...
            for (unsigned int j = 0; j < 6; j++)
                deltav2[j]
                    += double(h_virial_7.data[j * pitch + i] - h_virial_6.data[j * pitch + i])
                       * double(h_virial_7.data[j * pitch + i] - h_virial_6.data[j * pitch + i]);
