...
typedef std::function<std::shared_ptr<TableAngleForceCompute>(
    std::shared_ptr<SystemDefinition> sysdef,
    unsigned int width)>
    angleforce_creator;

//! Perform some simple functionality tests of any BondForceCompute
void angle_force_basic_tests(angleforce_creator tf_creator,
    ...
    /*
       ...
       h_pos.data[2].x, h_pos.data[2].y, h_pos.data[2].z); printf(" Particle 3: x = %f  y = %f  z =
...
std::shared_ptr<TableAngleForceCompute>
base_class_tf_creator(std::shared_ptr<SystemDefinition> sysdef, unsigned int width)
    {
...
UP_TEST(TableAngleForceCompute_basic)
    ...
    angle_force_basic_tests(tf_creator,
