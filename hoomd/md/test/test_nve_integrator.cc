...
void nve_updater_integrate_tests(twostepnve_creator nve_creator,
                                 std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    // check that the nve updater can actually integrate particle positions and velocities correctly
    // start with a 2 particle system to keep things simple: also put everything in a huge box so
    // boundary conditions don't come into play
    std::shared_ptr<SystemDefinition> sysdef(
        new SystemDefinition(2, BoxDim(1000.0), 4, 0, 0, 0, 0, exec_conf));
...
void nve_updater_limit_tests(twostepnve_creator nve_creator,
                             std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    // create a simple 1 particle system
    std::shared_ptr<SystemDefinition> sysdef(
        new SystemDefinition(1, BoxDim(1000.0), 1, 0, 0, 0, 0, exec_conf));
    std::shared_ptr<ParticleData> pdata = sysdef->getParticleData();
    std::shared_ptr<ParticleFilter> selector_all(
        new ParticleFilterTag(sysdef, 0, pdata->getN() - 1));
    std::shared_ptr<ParticleGroup> group_all(new ParticleGroup(sysdef, selector_all));
...
void nve_updater_boundary_tests(twostepnve_creator nve_creator,
                                std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    ////////////////////////////////////////////////////////////////////
    ...
    std::shared_ptr<ParticleFilter> selector_all(
        new ParticleFilterTag(sysdef_6, 0, pdata_6->getN() - 1));
    std::shared_ptr<ParticleGroup> group_all(new ParticleGroup(sysdef_6, selector_all));
        ...
        ArrayHandle<Scalar4> h_pos(pdata_6->getPositions(),
                                   access_location::host,
                                   access_mode::readwrite);
        ArrayHandle<Scalar4> h_vel(pdata_6->getVelocities(),
                                   access_location::host,
                                   access_mode::readwrite);
...
void nve_updater_compare_test(twostepnve_creator nve_creator1,
    ...
    std::shared_ptr<ParticleFilter> selector_all1(
        new ParticleFilterTag(sysdef1, 0, pdata1->getN() - 1));
    ...
    std::shared_ptr<ParticleFilter> selector_all2(
        new ParticleFilterTag(sysdef2, 0, pdata2->getN() - 1));
    std::shared_ptr<ParticleGroup> group_all2(new ParticleGroup(sysdef2, selector_all2));

    std::shared_ptr<NeighborListTree> nlist1(
        new NeighborListTree(sysdef1, Scalar(3.0), Scalar(0.8)));
    std::shared_ptr<NeighborListTree> nlist2(
        new NeighborListTree(sysdef2, Scalar(3.0), Scalar(0.8)));
    ...
    for (int i = 0; i < 5; i++)
        ...
        std::cout << "i = " << i << std::endl;
            {
            ArrayHandle<Scalar4> h_pos1(pdata1->getPositions(),
                                        access_location::host,
                                        access_mode::read);
            ArrayHandle<Scalar4> h_vel1(pdata1->getVelocities(),
                                        access_location::host,
                                        access_mode::read);
            ArrayHandle<Scalar3> h_accel1(pdata1->getAccelerations(),
                                          access_location::host,
                                          access_mode::read);
            ArrayHandle<Scalar4> h_pos2(pdata2->getPositions(),
                                        access_location::host,
                                        access_mode::read);
            ArrayHandle<Scalar4> h_vel2(pdata2->getVelocities(),
                                        access_location::host,
                                        access_mode::read);
            ArrayHandle<Scalar3> h_accel2(pdata2->getAccelerations(),
                                          access_location::host,
                                          access_mode::read);
...
void nve_updater_aniso_test(std::shared_ptr<ExecutionConfiguration> exec_conf,
                            twostepnve_creator nve_creator)
    {
    ...
    std::shared_ptr<ComputeThermo> thermo_1
        = std::shared_ptr<ComputeThermo>(new ComputeThermo(sysdef_1, group_all_1));
    ...
    std::cout << "Measuring conserved quantity for another " << n_measure_steps << " time steps..."
              << std::endl;
...
UP_TEST(TwoStepNVE_aniso_test)
    {
    nve_updater_aniso_test(std::shared_ptr<ExecutionConfiguration>(
                               new ExecutionConfiguration(ExecutionConfiguration::CPU)),
                           bind(base_class_nve_creator, _1, _2));
    }
...
UP_TEST(TwoStepNVEGPU_comparison_tests)
    ...
    twostepnve_creator nve_creator_gpu = bind(gpu_nve_creator, _1, _2);
    twostepnve_creator nve_creator = bind(base_class_nve_creator, _1, _2);
    nve_updater_compare_test(nve_creator,
                             nve_creator_gpu,
                             std::shared_ptr<ExecutionConfiguration>(
                                 new ExecutionConfiguration(ExecutionConfiguration::GPU)));
    }
