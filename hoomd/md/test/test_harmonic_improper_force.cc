...
typedef std::function<std::shared_ptr<HarmonicImproperForceCompute>(
    std::shared_ptr<SystemDefinition> sysdef)>
    improperforce_creator;

//! Perform some simple functionality tests of any BondForceCompute
void improper_force_basic_tests(improperforce_creator tf_creator,
                                std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    /////////////////////////////////////////////////////////
    // start with the simplest possible test: 4 particles in a huge box with only one improper type
    // !!!! NO IMPROPERS
    std::shared_ptr<SystemDefinition> sysdef_4(
        new SystemDefinition(4, BoxDim(1000.0), 1, 0, 0, 0, 1, exec_conf));
    ...
    */
    ...
    fc_4->compute(0);
        ...
        // check that the force is correct, it should be 0 since we haven't created any impropers
        // yet
    ...
    std::shared_ptr<SystemDefinition> sysdef_8(
        new SystemDefinition(8, BoxDim(60.0, 70.0, 80.0), 1, 0, 0, 0, 2, exec_conf));
    ...
    std::shared_ptr<SystemDefinition> sysdef_5(
        new SystemDefinition(5, BoxDim(100.0, 100.0, 100.0), 1, 0, 0, 0, 1, exec_conf));
...
void improper_force_comparison_tests(improperforce_creator tf_creator1,
    ...
    fc2->compute(0);
        ...
        for (unsigned int i = 0; i < N; i++)
            {
            deltaf2 += double(h_force_7.data[i].x - h_force_6.data[i].x)
                       * double(h_force_7.data[i].x - h_force_6.data[i].x);
            deltaf2 += double(h_force_7.data[i].y - h_force_6.data[i].y)
                       * double(h_force_7.data[i].y - h_force_6.data[i].y);
            deltaf2 += double(h_force_7.data[i].z - h_force_6.data[i].z)
                       * double(h_force_7.data[i].z - h_force_6.data[i].z);
            deltape2 += double(h_force_7.data[i].w - h_force_6.data[i].w)
                        * double(h_force_7.data[i].w - h_force_6.data[i].w);
...
std::shared_ptr<HarmonicImproperForceCompute>
...
gpu_tf_creator(std::shared_ptr<SystemDefinition> sysdef)
    {
    return std::shared_ptr<HarmonicImproperForceCompute>(
        new HarmonicImproperForceComputeGPU(sysdef));
    }
#endif
...
UP_TEST(HarmonicImproperForceCompute_basic)
    ...
    improper_force_basic_tests(tf_creator,
...
UP_TEST(HarmonicImproperForceComputeGPU_compare)
    ...
    improperforce_creator tf_creator = bind(base_class_tf_creator, _1);
    improper_force_comparison_tests(tf_creator,
                                    tf_creator_gpu,
                                    std::shared_ptr<ExecutionConfiguration>(
                                        new ExecutionConfiguration(ExecutionConfiguration::GPU)));
    }
